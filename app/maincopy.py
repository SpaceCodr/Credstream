...
def embed_watermark(section, logo,a,b,z1,z2):
    ...
    coeffs_R = (LowPassR, (LH_R, HL_R, HH_R))
    recon_img_R = pywt.idwt2(coeffs_R, wavelet)

    coeffs_G = (LowPassG, (LH_G, HL_G, HH_G))
    recon_img_G = pywt.idwt2(coeffs_G, wavelet)

    coeffs_B = (LowPassB, (LH_B, HL_B, HH_B))
    recon_img_B = pywt.idwt2(coeffs_B, wavelet)
...
def extract_watermark(section1,a,b,z1,z2):
    section1 = cv2.cvtColor(section1,cv2.COLOR_RGB2YCrCb)
    R = section1[:, :, 0]
    G = section1[:, :, 1]
    B = section1[:, :, 2]
    ...
    # Apply CT to each channel
    wavelet = 'db2'
    coeffs_R1 = pywt.dwt2(R, wavelet)
    coeffs_G1 = pywt.dwt2(G, wavelet)
    coeffs_B1 = pywt.dwt2(B, wavelet)
    ...
    # Combine LL sub-bands of each channel
    LL1 = cv2.merge((LL_R1, LL_G1, LL_B1))

    w_x, w_y = 64,64
    ll_x, ll_y = LL1.shape[0], LL1.shape[1]

    mask = np.zeros(shape=((ll_x//4),(ll_y//4),3))
    watermark = np.zeros(shape=(64,64))
...
def Extract(input_file):
    ...
    accumulated = np.zeros(shape=(64,64))
